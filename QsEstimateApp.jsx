...
const appStorage = BACKEND_URL ? {
  get: async (key) => {
    try {
      const r = await fetch(`${BACKEND_URL}/api/storage/${key}`, { headers: { "x-user-id": getUserId() } });
      ...
      if (BACKEND_URL) {
        ...
        if (data.jobId && !Array.isArray(data.items)) {
          ...
          while (trangThaiTong === "dang_chay" && soVongCho < 200) { // tối đa ~200×3s = 10 phút chờ
            ...
            let resStatus;
            try {
              // Timeout 15s trên CHÍNH lệnh poll — trước đây fetch() ở đây
              // không có timeout, nếu 1 lượt poll bị treo (mạng rớt lặng lẽ),
              // vòng lặp while KHÔNG BAO GIỜ tăng soVongCho được nữa vì đang
              // "await" mãi ở dòng fetch — trần "200 vòng x 3s = 10 phút" chỉ
              // là lý thuyết, thực tế có thể treo vô thời hạn đúng y hệt lỗi
              // gốc đang sửa. Giờ 1 lượt poll hỏng tối đa mất 15s rồi tự huỷ,
              // rơi vào catch, thử lại vòng sau — trần 10 phút giờ có thật.
              resStatus = await fetchCoTimeout(`${BACKEND_URL}/api/jobs/${jobId}/status`, { headers: { "x-user-id": getUserId(), ...authHeaders() } }, 15000);
            } catch (e) { continue; } // lỗi mạng tạm thời khi poll — thử lại vòng sau, không dừng hẳn
