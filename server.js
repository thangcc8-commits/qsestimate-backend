...
// ============================================================================
// PHỤC VỤ CHÍNH ỨNG DỤNG (index.html) — mở địa chỉ gốc của server là dùng được app
// ngay trên trình duyệt, không cần Claude.ai, không cần tải file. Vì app và backend
// cùng một địa chỉ nên không bao giờ bị chặn gọi lẫn nhau (không có vấn đề CORS).
// ============================================================================
app.get("/", (req, res) => {
  const file = path.join(__dirname, "index.html");
  if (fs.existsSync(file)) return res.sendFile(file);
  res.status(404).send("Chưa có file index.html trong thư mục server — tải file index.html lên GitHub cùng chỗ với server.js.");
});
